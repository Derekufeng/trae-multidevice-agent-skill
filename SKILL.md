...
### Git PATH 问题
Windows 安装 Git for Windows 后，`git` 可能不在系统 PATH 中。需手动添加：
```powershell
$env:PATH = "C:\Program Files\Git\bin;C:\Program Files\Git\cmd;$env:PATH"
```
或将 `C:\Program Files\Git\cmd` 添加到系统环境变量 PATH 中（永久生效）。

### SSH 端口 22 被拒绝
部分网络环境（公司防火墙等）会封禁 SSH 22 端口。解决方法：配置 SSH 使用 443 端口。
创建 `~/.ssh/config` 文件（无 BOM 的 UTF-8 编码）：
```
Host github.com
...
### 身份文件同步
本技能同步 `~/.trae-cn/skills/` 目录下的技能文件。以下身份文件不在自动同步范围内，需手动在各设备创建或同步：
- `~/.trae-cn/SOUL.md` — 核心价值观
- `~/.trae-cn/IDENTITY.md` — 助手身份定义
- `~/.trae-cn/USER.md` — 用户信息
- `~/.trae-cn/memory/user_profile.md` — 用户偏好记忆

如需同步身份文件，可将它们放入 `skills/` 目录下统一管理，或手动复制。

### 首次推送冲突处理
远程仓库已有内容时，`git push` 会被拒绝。需先合并：
```powershell
git pull origin main --allow-unrelated-histories --no-edit
# 冲突文件保留本地版本
git checkout --ours -- .
git add -A
git commit -m "merge remote config"
